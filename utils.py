...
def ASR_PA(model, features, perturbed_features, labels, idx_test, adj, perturbed_adj, device, feature_modify=False, adj_modify=True):

    num_classes = labels.max().item() + 1
    asr_list = []

    # original output
    perturbed_adj, features, labels = to_tensor(perturbed_adj, features, labels, device=device)
    adj = to_tensor(adj=adj, device=device)
    adj = adj.to_dense()
    # ori_output = model.predict(features, adj)
    ori_adj_norm, _ = normalize_adj_tensor(adj)
    ...
    adj_norm, _ = normalize_adj_tensor(adj)


    hidden = features
    hidden_2 = features

    # Output with perturbed adj
    for ix, w in enumerate(model.weights):
        b = model.biases[ix] if model.with_bias else 0
        if model.sparse_features:
            hidden = adj_norm @ torch.spmm(hidden, w) + b
        else:
            hidden = adj_norm @ hidden @ w + b
        if model.with_relu and ix != len(model.weights) - 1:
    ...
    for ix, w in enumerate(model.weights):
        b = model.biases[ix] if model.with_bias else 0
        if model.sparse_features:
            hidden_2 = ori_adj_norm @ torch.spmm(hidden_2, w) + b
        else:
            hidden_2 = ori_adj_norm @ hidden_2 @ w + b
        if model.with_relu and ix != len(model.weights) - 1:
            hidden_2 = F.relu(hidden_2)
    output_ori = F.log_softmax(hidden_2, dim=1)
    output_ori_label = torch.argmax(output_ori, dim=1)

    asr = 1 - ((output_label[idx_test] == output_ori_label[idx_test]).sum() / output_label[idx_test].shape[0])
    asr = float(asr.detach().cpu().numpy())
...
def ASR_PGD_PA(model, features, perturbed_features, labels, idx_test, adj, perturbed_adj, device, feature_modify=False, adj_modify=True):
    ...
    # Output with perturbed adj
    model.eval()
    output = model(hidden, adj_norm)
    output_label = torch.argmax(output, dim=1)



    # Output with original adj
    output_ori = model(hidden_2, ori_adj_norm)
    output_ori_label = torch.argmax(output_ori, dim=1)
...
def ASR_EA(model, features, perturbed_features, labels, idx_test, adj, perturbed_adj, device, feature_modify=False, adj_modify=True):
    ...
    for i in range(1):
        ...
        for j in range(num_classes):
            new_pred.append(0)
        for k in idx_test:
            output = model.predict(features, adj)  # (2485, 7)
        ...
        asr = float(sum(res)/len(res))
        asr_list.append(asr)

    average_asr = sum(asr_list)/float(len(asr_list))
    print('EA ASR is', average_asr)
    return average_asr


def pa_predict(model, features, adj):
    if features is None:
        pass
    else:
        if torch.is_tensor(features):
            ...
            if sp.issparse(features):
                features_tmp = sparse_mx_to_torch_sparse_tensor(features)
            else:
                features_tmp = torch.FloatTensor(np.array(features))
    hidden = features_tmp.to(model.device)
    adj_norm, _ = normalize_adj_tensor(adj)
    for ix, w in enumerate(model.weights):
        b = model.biases[ix] if model.with_bias else 0
        if model.sparse_features:
...
def to_tensor(adj=None, features=None, labels=None, device='cpu'):
    ...
    if (adj is None) and  (features is None) and (labels is None):
        raise Exception("All input are None!")
